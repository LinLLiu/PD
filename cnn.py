...
def MCC(y_true, y_pred):
    ...
    if (y_true - y_pred) ==0:
        y_true = 1
        y_pred = 1
        TN = K.sum(K.round(K.clip(y_true * y_pred, 0,1))) #TN
    else:
        pass
    FN = K.sum(K.round(K.clip(y_true, 0, 1)))  #FN
    FP = K.sum(K.round(K.clip(y_pred, 0, 1)))   #FP
    A = (FP*FN)-(TP*TN)
    B = K.sqrt((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN) + K.epsilon())
    mcc = A/B
    return mcc
...
y=y_trainOneHot, validation_data=(X_test4D_normalize, y_testOneHot),epochs=2500, batch_size=50,verbose=2)

#show_train_history(train_history,'f1','val_f1')
#show_train_history(train_history,'acc','val_acc')
#show_train_history(train_history,'MCC','val_MCC')
